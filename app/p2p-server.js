...
class P2pserver{
    ...
    sendChain(socket){                                                          //Send local chain to socket
        //socket.send(JSON.stringify({this.blockchain.chain}));
        socket.send(JSON.stringify(this.blockchain.chain));
    }

    syncChain(){                                                                //Used in index.js to sync the chains
        this.sockets.forEach(socket =>{                                         //when a new block is added to the blockchain
            this.sendChain(socket);
        });
    }
